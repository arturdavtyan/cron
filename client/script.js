...
saveBtn.addEventListener("click", async () => {
  const data = allRows.map(
    ({ name, username, email, password, role, company }) => {
      return {
        name: name.value,
  ...
  const response = await fetch(
    "http://localhost:5000/api/v1/users/createMany",
    {
      method: "POST",
...
const getAllUsers = async (filter = {}) => {
  const params =
    Object.keys(filter).reduce((acc, key, i) => {
...
const drawCompanyRow = () => {
  const rowWrapper = document.createElement("div");
  rowWrapper.classList.add("row-wrapper");
  const companyNameInput = document.createElement("input");
  companyNameInput.placeholder = "Name";
  const companyAddressInput = document.createElement("input");
  companyAddressInput.placeholder = "Address";
  const companyTypeInput = document.createElement("input");
  companyTypeInput.placeholder = "Type";
...
const addCompanyWithUsersBtn = document.getElementById("addCompanyWithUsers");

addCompanyWithUsersBtn.addEventListener("click", () => {
  drawCompanyRow();
});

getAllUsers({
  limit: document.getElementById("usersPerPage").value,
  offset: 1,
});
